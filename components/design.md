...
Let’s create a simple card with a profile image and a name and turn it into a design component. If you select the master, you will see a section Row / Master the properties panel with two label fields, a text and image field. Name the text field `Name` and the profile `Avatar` and enable them as properties.
